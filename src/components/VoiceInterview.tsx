...
export function VoiceInterview({
  ...
  const getAIResponse = useCallback(async (userMessage: string) => {
    ...
    try {
      ...
      if (userMessage.toLowerCase().includes("end interview")) {
        await speak(reply);
        setTimeout(() => onEndInterview(), 15000);
      } else {
        await speak(reply);
      }
    } catch (error) {
  ...
  }, [messages, resumeText, experienceLevel, jobDescription, speak, toast, onEndInterview]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        ...
        {!isConnected ? (
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground">
              Click the button below to start your voice interview with the AI interviewer.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
      ...
      <p className="text-center text-xs text-muted-foreground mt-8 max-w-md">
        {isConnected
          ? 'Speak clearly when the microphone is active. Say "end interview" to finish and receive feedback.'
          : "Make sure your microphone is enabled. The interview will be conducted entirely by voice."}
      </p>
    </div>
  );
}
